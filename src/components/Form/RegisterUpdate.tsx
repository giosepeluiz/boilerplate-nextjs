...
const RegisterUpdate = ({ register }) => {
  const initialState = {
    date: `${String(register.year)}-${String(register.month).padStart(2, "0")}-${String(
      register.day,
    ).padStart(2, "0")}`,
  ...
  async function handleSubmit(e) {
    ...
    const newDay = Number(target.elements.namedItem("date").value.split("-")[2]);
    const newMonth = Number(target.elements.namedItem("date").value.split("-")[1]);
    const newYear = Number(target.elements.namedItem("date").value.split("-")[0]);
    const newHoliday = target.elements.namedItem("holiday").checked;
    const newDinner = target.elements.namedItem("dinner").checked;
    const newStartTime = target.elements.namedItem("startTime").value;
    const newEndTime = target.elements.namedItem("endTime").value;
    const newReason = target.elements.namedItem("reason").value;
    const newLocation = target.elements.namedItem("location").value;
    const newTeam = target.elements.namedItem("team").value;
    const newTotalTime = calcOvertime(
      newStartTime,
      newEndTime,
      false,
      newHoliday,
    ...
    const newOverTime = calcOvertime(
      newStartTime,
      newEndTime,
      true,
      newHoliday,
    ...
    try {
      ...
      const res = await fetch(`${process.env.HOST}/data/${register._id}`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        router.push("/manager?message=updated");
      } else {
              ...
              <div className={styles.cell}>
                <label htmlFor="holiday">
                  <input
                    ...
                    name="holiday"
                    onChange={setFormData}
                    defaultChecked={formData.holiday}
                  />
                  Sábado ou Feriado
                </label>
              </div>
              <div className={styles.cell}>
                <label htmlFor="dinner">
                  <input
                    ...
                    onChange={setFormData}
                    name="dinner"
                    defaultChecked={formData.dinner}
                  />
                  Almoço Trabalhado
                </label>
              </div>
